...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
  if ([@"cropImage" isEqualToString:call.method]) {
      ...
      [[self topFlutterViewController:[self keyUIWindow]] presentViewController:cropViewController animated:YES completion:nil];
  } else {
...
//get flutterviewcontroller by window
- (UIViewController *)topFlutterViewController:(UIWindow *)window {
  UIViewController *topController = window.rootViewController;
  
  //用一个变量把所有pressented的vc存起来
  NSMutableArray* pressentedController = [NSMutableArray arrayWithCapacity:0];
    
  [pressentedController insertObject:topController atIndex:0];
    
  while (topController.presentedViewController) {
    topController = topController.presentedViewController;
    [pressentedController insertObject:topController atIndex:0];
  }
    for (UIViewController *controller in pressentedController) {
        ///如果当前VC没有被dismiss 则取当前的
